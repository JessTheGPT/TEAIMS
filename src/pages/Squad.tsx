...
const Squad = () => {
  ...
  const [activeAgent, setActiveAgent] = useState(SQUAD_CHIEF.id);
  ...
  const nextPipelineAgent = SQUAD_AGENTS[Math.min(currentStep, SQUAD_AGENTS.length - 1)];
  const allAgentsDone = completedAgents.size >= SQUAD_AGENTS.length;

  const ideaBrief = activeIdea
    ? `THE IDEA\nName: ${activeIdea.title}\n${activeIdea.description ? `Founder's description: ${activeIdea.description}` : 'No description provided yet — ask the founder for it.'}`
    : '';

  const pipelineState = () => {
    const done = SQUAD_AGENTS.filter(a => completedAgents.has(a.id)).map(a => a.name);
    return `PIPELINE STATE\nCompleted: ${done.length ? done.join(', ') : 'nothing yet'}\nNext specialist: ${allAgentsDone ? 'none — all nine are done' : `${nextPipelineAgent.name} (${nextPipelineAgent.role})`}`;
  };

  const chatContext = (agentId: string) => {
    const docs = documents
      .filter(d => d.status === 'complete' || d.status === 'reviewed')
      .map(d => `## ${d.title}\n\n${d.content}`)
      .join('\n\n');
    const parts = [ideaBrief];
    if (agentId === SQUAD_CHIEF.id) parts.push(pipelineState());
    if (docs) parts.push(`WORK COMPLETED SO FAR\n${docs}`);
    return parts.filter(Boolean).join('\n\n---\n\n');
  };

  const runNextPipelineAgent = () => {
    if (!activeIdea || generating || allAgentsDone) return;
    autoGenerateDocument(activeIdea, nextPipelineAgent.id, currentStep);
    setCurrentStep(prev => Math.min(prev + 1, SQUAD_AGENTS.length));
    addActivity({
      type: 'delegation',
      fromAgent: SQUAD_CHIEF.id,
      toAgent: nextPipelineAgent.id,
      content: `Chief of Staff handed the next step to ${nextPipelineAgent.name}`,
    });
  };
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <IdeaSelector
              ...
              onSelect={(idea) => { setActiveIdea(idea); setActiveAgent(SQUAD_CHIEF.id); setCenterTab('activity'); setViewingDocId(null); }}
              onNew={() => setShowNewDialog(true)}
              onRename={handleRenameIdea}
            />
          </div>
          <div className="flex items-center gap-1.5">
            {activeAgent === SQUAD_CHIEF.id && !allAgentsDone && (
              <Button onClick={runNextPipelineAgent} disabled={generating} size="sm" variant="outline" className="gap-1 text-[10px] h-7">
                {generating ? <><Loader2 className="w-2.5 h-2.5 animate-spin" />Working...</> : <>Run {nextPipelineAgent.name} <ArrowRight className="w-2.5 h-2.5" /></>}
              </Button>
            )}
            {activeAgent !== SQUAD_CHIEF.id && canAdvance && (
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="w-80 flex-shrink-0 border-r border-border/40 flex flex-col bg-card/20">
              ...
              <div className="flex-shrink-0 border-b border-border/40">
                <button
                  onClick={() => setActiveAgent(SQUAD_CHIEF.id)}
                  className={`w-full flex items-center gap-2 px-2.5 py-2 text-left transition-colors ${
                    activeAgent === SQUAD_CHIEF.id ? 'bg-primary/10' : 'hover:bg-muted/30'
                  }`}
                >
                  <span className="text-sm">{SQUAD_CHIEF.icon}</span>
                  <div className="min-w-0 flex-1">
                    <p className={`text-[11px] font-semibold truncate ${activeAgent === SQUAD_CHIEF.id ? 'text-primary' : 'text-foreground'}`}>
                      {SQUAD_CHIEF.name}
                    </p>
                    <p className="text-[9px] text-muted-foreground truncate">{SQUAD_CHIEF.role}</p>
                  </div>
                  {!allAgentsDone && (
                    <span className="text-[8px] text-muted-foreground flex-shrink-0">Next: {nextPipelineAgent.icon}</span>
                  )}
                </button>
              </div>
              ...
              <div className="flex-1 overflow-hidden">
                <AgentChat
                  ...
                  agent={activeAgent}
                  context={chatContext(activeAgent)}
                  messages={agentMessages[activeAgent] || []}
                  onMessagesChange={(msgs) => handleMessagesChange(activeAgent, msgs)}
                  onReadyToAdvance={() => {
                    if (activeAgent === SQUAD_CHIEF.id) {
                      runNextPipelineAgent();
                      toast.success(`Chief of Staff started ${nextPipelineAgent.name}`);
                    } else {
                      toast.info('Agent is ready to advance. Click "Next Agent" to proceed.');
                    }
                  }}
                />
              </div>
